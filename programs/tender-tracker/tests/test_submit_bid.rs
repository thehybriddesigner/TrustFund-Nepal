...
fn test_submit_bid() {
    ...
    let tender_id: u64 = 1;
    let tender = Pubkey::find_program_address(&[b"tender", tender_id.to_le_bytes().as_ref()],
    &program_id,
    )
    .0;
    let bid = Pubkey::find_program_address(
    &[b"bid", tender_id.to_le_bytes().as_ref(), bidder.pubkey().as_ref()],
    &program_id,
    )
    .0;
...
let blockhash = svm.latest_blockhash();
let msg = Message::new_with_blockhash(&[submit_ix], Some(&bidder.pubkey()), &blockhash);
let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&bidder]).unwrap();
let res = svm.send_transaction(tx);
assert!(res.is_ok());

let bid_account = svm.get_account(&bid).unwrap();
let mut bid_data:&[u8]  = &bid_account.data;
let bid_state = tender_tracker::state::Bid::try_deserialize(&mut bid_data).unwrap();
assert_eq!(bid_state.price, 20000000);
assert_eq!(bid_state.bidder, bidder.pubkey());
